...
namespace IDS.OWID.Service.QrCode.Code
{
  public static class QrCodeBitmapExtensions
  {
    ...
    public static SKBitmap ToBitmap(this Net.Codecrete.QrCodeGenerator.QrCode qrCode, int scale, int border, SKColor foreground, SKColor background)
    {
      // check arguments
      if (scale <= 0)
      {
    ...
    public static SKBitmap ToBitmap(this Net.Codecrete.QrCodeGenerator.QrCode qrCode, int scale, int border)
    {
    ...
    /// <param name="background">The background color.</param>
    /// <param name="foreground">The foreground color.</param>
    public static byte[] ToPng(this Net.Codecrete.QrCodeGenerator.QrCode qrCode, int scale, int border, SKColor foreground, SKColor background)
    {
    ...
    /// <exception cref="ArgumentOutOfRangeException"><paramref name="scale"/> is 0 or negative, <paramref name="border"/> is negative
    /// or the resulting image is wider than 32,768 pixels.</exception>
    public static byte[] ToPng(this Net.Codecrete.QrCodeGenerator.QrCode qrCode, int scale, int border)
    {
    ...
    /// <param name="background">The background color.</param>
    /// <param name="foreground">The foreground color.</param>
    public static void SaveAsPng(this Net.Codecrete.QrCodeGenerator.QrCode qrCode, string filename, int scale, int border, SKColor foreground, SKColor background)
    {
      using SKBitmap bitmap = qrCode.ToBitmap(scale, border, foreground, background);
      using SKData data = bitmap.Encode(SKEncodedImageFormat.Png, 90);
    ...
    /// <exception cref="ArgumentOutOfRangeException"><paramref name="scale"/> is 0 or negative, <paramref name="border"/> is negative
    /// or the resulting image is wider than 32,768 pixels.</exception>
    public static void SaveAsPng(this Net.Codecrete.QrCodeGenerator.QrCode qrCode, string filename, int scale, int border)
    {
